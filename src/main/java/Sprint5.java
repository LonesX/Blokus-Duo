...
import scheme.RandomBot;
import scheme.SchemeBot;
...
public class Sprint5 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int playerNo = 0; playerNo < players.length; playerNo++) {
            SchemeBot botPlayer = new SchemeBot(playerNo);
        ...
        if (graphicsUI) {
            ...
            if (SchemeBot.class.isAssignableFrom(players[0].getClass())) {
                ((SchemeBot)players[0]).setOpponent(players[1]);
            }

            if (SchemeBot.class.isAssignableFrom(players[1].getClass())) {
                ((SchemeBot)players[1]).setOpponent(players[0]);
            }
